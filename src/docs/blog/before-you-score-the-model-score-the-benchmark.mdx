...
# TL;DR
Agentic coding benchmarks are how the field measures whether AI agents can actually build software. Many of the standard ones have serious problems — most importantly, that the human-authored gold patch doesn't reliably resolve tasks, leaving every model score on top of it built on shifting ground.

- **Started with SWE-bench**, the de facto standard that mostly contains bug-fixing tasks, and found a moving Oracle ceiling, a saturated leaderboard, and a task format that doesn't match how developers actually use coding agents.
- **Surveyed feature implementation (FI) benchmarks** as a successor and found several from different groups (with confusingly similar names: FeatureBench, FEA-Bench, FeatBench), each with their own pros and cons. We went ahead with **FeatBench**.
- **FeatBench had a stability problem of its own**: Oracle resolution started at just **37.31%**, far below the ~100% you'd expect from ground truth. We worked through the harness bugs, fixed environment issues, curated a stable subset, and built an adapter for **Harbor** — a standardized platform for running agents against benchmarks. End state: **99.81%** Oracle resolution, with 99%+ of the original instances and tests preserved.
- **Released everything**: FeatBench-Verified v1.3, a Harbor adapter for FeatBench, our patched upstream harness, and PRs back to the original FeatBench repo.
- **Looking ahead**: if benchmark creators do one thing differently, it should be validating every benchmark with an Oracle (gold patch) and a no-op (no operation) agent first — confirming the ceiling lands near 100% and the floor is 0% before scoring any real model. And more broadly, infrastructure noise needs to be treated as a first-class problem, not something downstream evaluators are left to work around.
...
Many thanks to **Boyuan Chen**, **Gustavo Oliva**, **Ahmed E. Hassan**, and others on our team for their help on this work and for thoughtful feedback on earlier drafts of this post.

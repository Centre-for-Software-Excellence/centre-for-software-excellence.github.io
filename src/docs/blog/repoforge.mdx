...
The problem gets worse with reinforcement learning and rejection sampling, where many containers run in parallel. For example, 128 concurrent tasks at 7 GB each need around 896 GB just for runtime. Running all 7,304 tasks with both base and runtime images would require around <span className="font-bold text-red-500">61 TB</span>, far beyond typical hardware.
